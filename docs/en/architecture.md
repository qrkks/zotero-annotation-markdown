...
`extensions.annotationMarkdown.outlineEnabled` defaults to `true` and is exposed as the user-facing visibility switch. Only an explicit outline-toggle click writes the separate `extensions.annotationMarkdown.outlineExpanded` state; disabling the feature, temporary absence, or editing never changes that stored choice. The visibility preference is observed with the other live Reader settings. One scroll listener tracks the active heading while the fixed portal remains available even after its selected row leaves the viewport; resize observation recomputes its side and dimensions. Heading activation computes a target within the actual sidebar scroller, avoiding document-page scrolling and the native annotation-selection path.
...
- Page annotation popup rendering is experimental and controlled by `extensions.annotationMarkdown.popupEnabled`, which defaults to `false`. When disabled, the controller removes the document-level `data-annotation-markdown-popup-enabled` marker, cancels positioning work, clears plugin-owned popup state, and leaves popup sizing, positioning, visibility, and editing to Zotero. When enabled, popups keep their native `.editor .content` source and bypass sidebar visibility-lazy rendering because Zotero can reuse popup DOM for another annotation. Popup-only CSS is scoped below the document marker. A stable `min(32em, calc(100vw - 3em))` popup width is present for the host's first layout, and padded long previews own a viewport-bounded vertical scroller whose native scrollbar gestures are excluded from edit entry. A popup is transparent and non-interactive until it has a namespaced ready marker. Markdown rendering finishes synchronously before the controller's zero-delay fallback. A host style write observed after Markdown rendering schedules reveal on the next paint; if transform, pointer direction, width, or height changes before that paint—or no post-render positioning signal is observed—the controller falls back to requiring two consecutive stable frames. A newly mounted popup, reused source ID, or host-owned child replacement starts a new positioning cycle—even when the annotation ID is unchanged—while plugin-owned preview mutations and active editing are excluded. The plugin hides the complete dormant `.editor` shell beside its owned preview. With fast editing available, a regular content click mounts the same plugin textarea, preserves the visible preview height, commits once through the annotation manager, and reconciles the popup representation even when a sidebar copy of the same annotation exists. Disabled or unavailable fast editing restores the native popup editor.
