...
- Page annotation popups keep their native `.editor .content` source. They bypass sidebar visibility-lazy rendering because Zotero can reuse popup DOM for another annotation. A stable `min(32em, calc(100vw - 3em))` popup width is present for the host's first layout, and padded long previews own a viewport-bounded vertical scroller whose native scrollbar gestures are excluded from edit entry. While the plugin stylesheet is active, a popup is transparent and non-interactive until it has a namespaced ready marker. Markdown rendering finishes synchronously before the controller's zero-delay stabilization task. After Zotero's queued `updatePopupPosition()` measures the final preview, the controller samples transform, pointer direction, width, and height until they match across two consecutive animation frames; any intervening host style update cancels the pending reveal and restarts stabilization. A newly mounted popup, reused source ID, or host-owned child replacement starts a new positioning cycle—even when the annotation ID is unchanged—while plugin-owned preview mutations and native editing are excluded. The plugin hides the complete dormant `.editor` shell beside its owned preview, restores that native editor for regular content clicks, and never mounts the sidebar fast textarea in a popup.
