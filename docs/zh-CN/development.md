...
12. 展开大纲后，在符合条件的标注和 Reader 标签页间切换并重启 Zotero，下一次大纲仍应展开。在 Reader 保持打开时，从插件设置关闭再开启大纲：浮层应消失再恢复，且不能覆盖已记住的展开/收起选择。收起后重复检查；临时编辑或无标题标注同样不能覆盖该选择。
13. 分别用指针和键盘激活多个大纲条目，只应滚动标注侧栏，当前标题高亮随之变化，并且不能打开评论编辑器或清除选中状态。Reader 较宽时应在滚动条外侧向右展开，较窄时应回退到左侧且不能遮住滚动条；同时检查明暗主题。
