...
12. 关闭实验性弹窗渲染后，连续打开多个页内标注弹窗，确认其尺寸、定位、可见性和编辑器完全保持原生。再开启 **Render page annotation popups as Markdown (Experimental)**，连续打开多个弹窗，包括重复打开同一条标注、视口各边缘附近的标注和一条很长的 Markdown 评论：每个弹窗都应在 Zotero 按最终尺寸完成延迟测量前保持隐藏，随后只在最终原生位置出现一次，不暴露中间内容，不淡入，也不在可见状态下跳动；渲染后的宿主位置通常在下一次稳定绘制时显示，若几何尺寸随后仍有变化则必须回退到稳定检查。宽度应稳定加宽，并在较窄 Reader 窗口中自动收缩；复用的弹窗 DOM（包括同一 ID 下的宿主子节点替换）应渲染当前源码。预览应有清晰的内边距和可用的纵向滚动条，拖动、点击和释放滚动条都必须保持预览状态。开启快速编辑偏好时，点击普通内容应以预览高度打开一个文本框；输入不得更新原生编辑器，失焦或按 Esc 必须只保存一次并恢复 Markdown 预览。关闭快速编辑偏好后应回退原生弹窗编辑器。在 Reader 保持打开时关闭实验性弹窗渲染，确认就绪标记、预览、尺寸规则和弹窗编辑器替换均不再残留。聚焦编辑器后直接关闭弹窗时，已更改的草稿必须只保存一次，且不能阻止后续弹窗渲染；重新打开时不得出现重复预览或快速文本框。
