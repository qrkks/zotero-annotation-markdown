...
12. 连续打开多个页内标注弹窗，包括重复打开同一条标注、视口各边缘附近的标注和一条很长的 Markdown 评论：每个弹窗都应在 Zotero 按最终尺寸完成延迟测量且布局连续两个动画帧保持稳定前保持隐藏，随后只在最终原生位置出现一次，不暴露中间内容，不淡入，也不在可见状态下跳动；若宿主最终位置在这两个帧之间才到达，必须重新开始稳定计时。宽度应稳定加宽，并在较窄 Reader 窗口中自动收缩；复用的弹窗 DOM（包括同一 ID 下的宿主子节点替换）应渲染当前源码。预览应有清晰的内边距和可用的纵向滚动条；拖动、点击和释放滚动条都必须保持预览状态，点击普通内容才恢复 Zotero 原生弹窗编辑器；聚焦编辑器后直接关闭弹窗也不能阻止后续弹窗渲染；重新打开时不得出现重复预览或快速文本框。
