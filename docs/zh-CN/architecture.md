...
- 页内标注弹窗保留原生 `.editor .content` 源码。由于 Zotero 可能为另一条标注复用弹窗 DOM，弹窗绕过侧栏可见性懒渲染。稳定的 `min(32em, calc(100vw - 3em))` 宽度从宿主首次布局起就生效；带内边距的长预览自身提供受视口限制的纵向滚动，其原生滚动条手势不会进入编辑。插件样式生效期间，未带命名空间就绪标记的弹窗透明且不可交互。Markdown 会在控制器的零延迟兜底任务前同步完成。若在 Markdown 渲染后观察到宿主写入样式，控制器会安排在下一次绘制时显示；如果在绘制前位置、指针方向、宽度或高度继续变化，或者未观察到可靠的渲染后定位信号，则回退到要求连续两个稳定帧。重新挂载弹窗、复用源码 ID，或宿主持有的子节点被替换时都会开始新的定位周期，即使注释 ID 没有变化；插件自有预览变更和活跃编辑状态会被排除。插件在显示自有预览时隐藏完整的空闲 `.editor` 外壳。快速编辑可用时，点击普通内容会挂载同一个插件文本框，保持预览的可见高度，通过标注管理器只提交一次，并在同一批注的侧栏副本同时存在时仍协调当前弹窗；快速编辑被关闭或不可用时恢复原生弹窗编辑器。
