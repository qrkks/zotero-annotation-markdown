...
- 页内标注弹窗保留原生 `.editor .content` 源码。由于 Zotero 可能为另一条标注复用弹窗 DOM，弹窗绕过侧栏可见性懒渲染。稳定的 `min(32em, calc(100vw - 3em))` 宽度从宿主首次布局起就生效；带内边距的长预览自身提供受视口限制的纵向滚动，其原生滚动条手势不会进入编辑。插件样式生效期间，未带命名空间就绪标记的弹窗透明且不可交互。Markdown 会在控制器的零延迟稳定任务前同步完成。Zotero 排入队列的 `updatePopupPosition()` 按最终预览完成测量后，控制器持续采样位置、指针方向、宽度和高度，直到它们连续两个动画帧一致；期间任何宿主样式更新都会取消待执行的显示并重新开始稳定计时。重新挂载弹窗、复用源码 ID，或宿主持有的子节点被替换时都会开始新的定位周期，即使注释 ID 没有变化；插件自有预览变更和原生编辑状态会被排除。插件只在显示自有预览时隐藏完整的空闲 `.editor` 外壳，普通内容点击时恢复该原生编辑器，绝不在弹窗中挂载侧栏快速文本框。
